<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="学籍号" prop="xuejihao">
          <el-select  @change="xuejihaoChange" v-model="ruleForm.xuejihao" placeholder="请选择学籍号">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.xuejihao" label="学籍号" prop="xuejihao">
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="具体时间" prop="jutishijian">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.jutishijian" 
                type="date"
                placeholder="具体时间">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.jutishijian" label="具体时间" prop="jutishijian">
              <el-input v-model="ruleForm.jutishijian" 
                placeholder="具体时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.leixing = true;
          ...
          if(o=='jutishijian'){
            this.ruleForm.jutishijian = obj[o];
	    this.ro.jutishijian = true;
            continue;
          }
          if(o=='xiangxineirong'){
    ...
    xuejihaoChange () {
      this.$http({
        url: `follow/xuesheng/xuejihao?columnValue=`+ this.ruleForm.xuejihao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
    ...
    info(id) {
      this.$http({
        url: `xueshengjiangcheng/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.xiangxineirong = this.ruleForm.xiangxineirong.replace(reg,'../../../ssmm4dtg/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `xueshengjiangcheng/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.xueshengjiangchengCrossAddOrUpdateFlag = false;
                  this.parent.search();
