    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="名单" prop="mingdan">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.mingdan" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.mingdan" label="名单" prop="mingdan">
                    <span v-html="ruleForm.mingdan"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zongrenshu = true;
          ...
          if(o=='mingdan'){
            this.ruleForm.mingdan = obj[o];
	    this.ro.mingdan = true;
            continue;
          }
        }
      }
	...
	this.ruleForm.mingdan = this.ruleForm.mingdan.replace(reg,'../../../ssmm4dtg/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yuanxibanji/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yuanxibanjiCrossAddOrUpdateFlag = false;
                  this.parent.search();
