    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="培养计划" prop="peiyangjihua">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.peiyangjihua" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.peiyangjihua" label="培养计划" prop="peiyangjihua">
                    <span v-html="ruleForm.peiyangjihua"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.zhuangtai = true;
          ...
          if(o=='peiyangjihua'){
            this.ruleForm.peiyangjihua = obj[o];
	    this.ro.peiyangjihua = true;
            continue;
          }
        }
      }
	...
	this.ruleForm.peiyangjihua = this.ruleForm.peiyangjihua.replace(reg,'../../../ssmm4dtg/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `peiyangjihua/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.peiyangjihuaCrossAddOrUpdateFlag = false;
                  this.parent.search();
